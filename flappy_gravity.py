...
for i in range(600, -151, -10):
    for ii in range(-400, 600, 5):
        for iv in range(-80, 100, 10):
            q_table[(i, ii, iv)] = [0 for i in range(3)]
...
for i in range(num_ep):
    ...
    for j in range(2):
        pipes = get_random_pipes(SCREEN_WIDTH * j + 600)
    ...
    for k in range(10000):
        obs = (imp(pipe_group.sprites()[1].rect[0]), imp(pipe_group.sprites()[0].rect[1] - bird.rect[1]), bird.speed)
        ...
        new_obs = (imp(pipe_group.sprites()[1].rect[0]), pipe_group.sprites()[0].rect[1] - disc_pos(bird.rect[1]), bird.speed)
...
plt.scatter(np.arange(1, num_ep), scores, s = 0.1)
plt.grid()
